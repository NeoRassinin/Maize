...
def prepare_directories():
    """Создает необходимые директории для обучающей, валидационной и тестовой выборок."""
    for path in [TRAIN_IMAGES_PATH, TRAIN_MASKS_PATH, VAL_IMAGES_PATH, VAL_MASKS_PATH, TEST_IMAGES_PATH, TEST_MASKS_PATH]:
        os.makedirs(path, exist_ok=True)


def collect_dataset_paths():
    """Собирает все пути к изображениям и маскам из исходных папок."""
    subdirs = sorted([os.path.join(DATASET_PATH, d) for d in os.listdir(DATASET_PATH) if os.path.isdir(os.path.join(DATASET_PATH, d))])
    image_dirs = []
    mask_dirs = []
    for sd in subdirs:
        img, mask = sorted([os.path.join(sd, d) for d in os.listdir(sd) if os.path.isdir(os.path.join(sd, d))], reverse=True)
...
def cnt_num(number: int, max_zeros: int) -> str:
    """Генерирует строку с ведущими нулями по номеру изображения."""
    zeros = '0' * max_zeros
    while number > 0:
        number //= 10
        zeros = zeros[:-1]
...
def make_subimages_by_cropping(images_list, masks_list, crop_size, output_images, output_masks):
    """
    Разбивает изображения и маски на подизображения фиксированного размера.

    Args:
        images_list (list): Список путей к изображениям.
        masks_list (list): Список путей к маскам.
        crop_size (int): Размер обрезки.
        output_images (str): Папка для сохранения изображений.
        output_masks (str): Папка для сохранения масок.
    """
    for idx in range(len(masks_list)):
        ...
        for i in range(grid_w - 1):
            for j in range(grid_h - 1):
                transform = A.Compose([
                    A.Crop(x_min=i * crop_size, y_min=j * crop_size,
                           x_max=(i + 2) * crop_size, y_max=(j + 2) * crop_size, p=1)
                ])
                result = transform(image=image, mask=mask)
                number = grid_w * grid_h * idx + grid_h * i + j
                filename = f'{cnt_num(number, 5)}{number}'
                Image.fromarray(result['image']).save(f'{output_images}/{filename}.jpg')
                Image.fromarray(result['mask']).save(f'{output_masks}/{filename}.png')


def filter_small_contours(masks_dir, min_area=70):
    """Удаляет мелкие контуры с площадью меньше заданной."""
